...
def main():
    ...
    # create new model，建立模型，并且训练
        ...
        in_channels = len(args.modality)#in_channels是modality的长度，如果输入rgbd那么就是4通道。
        #这一边只提供了两个选择50或者18
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        result, img_merge = validate(val_loader, model, epoch) # evaluate on validation set，每次训练完以后都要测试一下
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):#处理被train_loader进去的每一个数据
...
def validate(val_loader, model, epoch, write_to_file=True):
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        if args.modality == 'd':
            ...
            elif (i < 8*skip) and (i % skip == 0):#对每隔skip张的图片进行信息的存储
                if args.modality == 'rgbd':
