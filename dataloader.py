...
def h5_loader(path):#读取数据
    h5f = h5py.File(path, "r")
    rgb = np.array(h5f['rgb'])#读取RGB
    rgb = np.transpose(rgb, (1, 2, 0))
    depth = np.array(h5f['depth'])#读取深度

     #guiyihua Leon

     #depth_input_cpu = np.squeeze(depth)
     #print("i am alive 00000000000000000000000000")
    d_min = np.max(depth)
    d_max = np.min(depth)
    # print("I am alive")
    delta=d_max-d_min
    depth=(depth-d_min)/delta
    rgb = rgb*255
    return rgb, depth
